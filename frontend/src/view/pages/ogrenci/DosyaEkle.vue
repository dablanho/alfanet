<template>
    <div>
        ...
        <div class="card">
            ...
            <div class="card-body" v-if="fetchStudentFiles.length">
                <div class="card-header">
                    <h4>Dosyalar</h4>
                </div>
                <div class="card-body">
                    <div class="col-12 col-sm-12">
                        ...
                        >
                            ...
                            <tbody>
                                <tr
                                    v-for="(value, key) in fetchStudentFiles"
                                    :key="key"
                                >
                                    <td>{{ value.ad }}</td>
                                    <td>
...
} from "@/core/services/store/ogrenci_dosya.module";
...
export default {
    ...
    methods: {
        ...mapActions({
            ...
            create_delete_file_action: CREATE_DELETE_STUDENT_FILE_ACTION,
            fetch_student_files_action: FETCH_STUDENT_FILES_ACTION
        }),
        submitForm() {
            let formData = new FormData();
            formData.append("ogrenci_id", this.$route.params.id);
            formData.append("ad", this.form.ad);
            formData.append("dosya", this.dosya);
            this.create_delete_file_action({
                url: "ogrenci/dosyalar/create",
                data: formData,
                msgStatus: false
            })
                .then(() => {
    ...
    mounted() {
        this.$store.dispatch(SET_BREADCRUMB, [
            ...
            {
                title: "Öğrenci Dosya İşlemleri"
            }
        ]);
        this.get_ogrenci(this.$route.params.id);
        this.fetch_student_files_action(this.$route.params.id);
    },
    computed: {
        ...mapGetters(["getOgrenci", "fetchStudentFiles"])
    }
};
</script>
